test_that("memo recovers true betas on clean bivariate data (no CHP)", {
  set.seed(42)
  K <- 80
  b_exp_1 <- rnorm(K, 0, 0.1)
  b_exp_2 <- rnorm(K, 0, 0.1)
  b_exp   <- cbind(b_exp_1, b_exp_2)
  se_exp  <- matrix(0.01, K, 2)
  theta1 <- 0.3; theta2 <- 0.5
  b_out_1 <- theta1 * b_exp_1 + rnorm(K, 0, 0.01)
  b_out_2 <- theta2 * b_exp_2 + rnorm(K, 0, 0.01)
